...
//		    found.getImages().clear();
			...
			Set<ProductImage> newImages = saveProductImages(dto); // Sử dụng phương thức saveProductImages bạn đã cung
																	// cấp
	...
	private Set<ProductImage> saveProductImages(ProductDto dto) {
		...
		var newList = dto.getImages().stream().map(item -> {
			...
			boolean isExisting = entityList.stream().anyMatch(
					existingImage -> existingImage.getId() != null && existingImage.getId().equals(item.getId()));
	...
	public void deleteProductById(Long id) {
		var found = productRepository.findById(id).orElseThrow(() -> new AppException("Product Not Found"));
	...
	public List<ProductBriefDto> getProductsRelate(Long id) {
		var list = productRepository.findByIdNot(id);
		var newList = list.stream().map(item -> {
			ProductBriefDto dto = new ProductBriefDto();
			BeanUtils.copyProperties(item, dto);
	...
	public Page<ProductBriefDto> getProductFilters(List<String> brand, List<String> kitType, List<String> gender,
			Double minPrice, Double maxPrice, Pageable pageable) {
		var list = productRepository.findByFilters(brand, kitType, gender, minPrice, maxPrice, pageable);
		var newList = list.stream().map(item -> {
			ProductBriefDto dto = new ProductBriefDto();
			BeanUtils.copyProperties(item, dto);
